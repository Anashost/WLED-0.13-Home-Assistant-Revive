...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    def async_update_entities():
        ...
        # 1. Spawn the Master Light (Base Entity)
...
class WLEDMasterLight(WledReviveEntity, LightEntity):
    """The Global Master Light (light.xx). Controls global state, but HAS full colors/effects."""
    def __init__(self, data, entry_id):
        super().__init__(data)
        self._attr_has_entity_name = True
        self._attr_name = None # Forces HA to use the base device name (light.xx)
    ...
    async def async_turn_on(self, **kwargs):
        ...
        # Native WLED uses an OBJECT (not an array with an ID) to target all selected segments natively!
        seg_payload = {} 
...
class WLEDSegmentLight(WledReviveEntity, LightEntity):
    """Segment Lights. Seg 0 is 'Main' (Brightness only). Seg X is 'Segment X' (Full RGB)."""
    def __init__(self, data, entry_id, segment_id):
        ...
        if segment_id == 0:
            ...
            # Native Behavior: Strip color/effects from Segment 0 so it doesn't fight the Master Light
            self._attr_supported_color_modes = {ColorMode.BRIGHTNESS}
            self._attr_color_mode = ColorMode.BRIGHTNESS
            # We omit supported_features here so it doesn't throw the 0 integer error
        else:
    ...
    def available(self) -> bool:
        ...
        # Native WLED Behavior: Hide "Main" if it is the only segment running
    ...
    async def async_turn_on(self, **kwargs):
        # Explicitly targets specific segment IDs so it never overwrites other presets
